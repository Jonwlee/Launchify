    //Credits to Costas Malamas for the javascript//
    ...
    function UpdateUnits(selectedProperty, units){
      // Updates the units displayed in the menu by selectedProperty
      var i;

      i = selectedProperty.selectedIndex;
      FillMenuWithArray(units, unit[i]);
    }

    function FillMenuWithArray(menu, array){
      // Fills the options of the menu with the elements of the array.
      
      var i;

      menu.length = array.length;
      for(i = 0; i < array.length; i++){
        menu.options[i].text = array[i];
      }
    }

    function checkInput(sourceForm, targetForm){
    ...
    function ConvertFromTo(sourceForm, targetForm){
      ...
      // (The input has been checked by the checkInput function).
    ...
    //-----------------------Event Handlers, get Elements------//


function changeHandlerSelect () {

  UpdateUnits(this, document.form_A.unit_menu); 
  UpdateUnits(this, document.form_B.unit_menu);
...
function clickHandlerConvertToB() {
  setTimeout(ConvertFromTo(this.form, document.form_B), 1);
  }

function clickHandlerConvertToA() {
  setTimeout(ConvertFromTo(this.form, document.form_A, 1));
  }

function main() {
  } 
    document.getElementById('convertToB').addEventListener('click', clickHandlerConvertToB );
    document.getElementById('convertToA').addEventListener('click', clickHandlerConvertToA );
    document.getElementById('clearInputs').addEventListener('click', clickHandlerClear );
    document.getElementById('the_menu').addEventListener("change", changeHandlerSelect);
